 ...
 * Searches YTS, TPB, EZTV, and 1337x for movie/TV torrents
 * Extracted from server routes into standalone service for Electron main process
 */
...
async function searchYTS(imdbId, title) {
    ...
    for (const mirror of mirrors) {
        try {
            ...
            if (imdbId) url += `query_term=${imdbId}`;
            else if (title) url += `query_term=${encodeURIComponent(title)}`;

            console.log(`[Torrents] Trying YTS mirror: ${mirror}`);
            const response = await fetchWithTimeout(url, 8000);
            const data = await response.json();

            if (data.status !== 'ok' || !data.data?.movies) continue;
...
async function searchEZTV(showName, season, episode) {
    try {
        ...
        if (season && episode) {
            ...
            const patterns = [`S${seasonStr}E${episodeStr}`, `${season}x${episodeStr}`, `${season}x${episode}`];
        ...
        return torrents.slice(0, 15).map(t => {
            ...
            return {
                quality, type: name.includes('WEB') ? 'web' : (name.includes('HDTV') ? 'hdtv' : 'other'),
                size: sizeStr, sizeBytes, seeds: parseInt(t.seeds) || 0, peers: parseInt(t.peers) || 0,
                magnetLink: t.magnet_url, hash: t.hash, source: 'EZTV', name
            };
        }).filter(t => t.magnetLink);
    } catch (error) {
...
/**
 * Search ThePirateBay via API proxy
 */
async function searchTPB(query) {
    try {
        ...
        if (!data || !Array.isArray(data) || data[0]?.id === '0') return [];
        ...
        return data.slice(0, 15).map(t => {
            ...
            return {
                quality, type: name.includes('BluRay') ? 'bluray' : (name.includes('WEB') ? 'web' : 'other'),
                size: sizeStr, sizeBytes, seeds: parseInt(t.seeders) || 0, peers: parseInt(t.leechers) || 0,
                magnetLink, hash: t.info_hash, source: 'TPB', name
            };
        }).filter(t => t.seeds > 0);
    } catch (error) {
...
function formatTorrentResult(t, source) {
    ...
    return {
        quality, type: name.includes('BluRay') ? 'bluray' : (name.includes('WEB') ? 'web' : 'other'),
        size: sizeStr, sizeBytes, seeds: parseInt(t.seeds || t.Seeds || t.seeders || 0) || 0,
        peers: parseInt(t.leeches || t.Leeches || t.leechers || 0) || 0,
        magnetLink: finalMagnet, hash, source, name
    };
}

/**
 * Main torrent search function
 * Searches multiple sources for movie/TV torrents
 */
async function searchTorrents({ imdbId, title, tmdbId, mediaType, season, episode }) {
    if (!imdbId && !title) {
        throw new Error('imdbId or title is required');
    }
    ...
    if (isTV && title) {
        ...
        // PRIORITY 1: Season packs
        const [tpbSeasonResult, x1337SeasonResult] = await Promise.allSettled([
            searchTPB(`${title} S${seasonStr} complete`).catch(() => []),
            searchTPB(seasonQuery).catch(() => []),
        ]);
        ...
        if (torrents.length === 0) {
            ...
            const [eztvResult, tpbResult] = await Promise.allSettled([
                searchEZTV(title, season, episode).catch(() => []),
                searchTPB(episodeQuery).catch(() => [])
            ]);
        ...
        // Movies: Try YTS first, then TPB, then 1337x
        const ytsResults = await searchYTS(imdbId, title);
        if (ytsResults.length > 0) torrents = ytsResults;
