...
# Backup script
cat > /opt/backup-cinestream.sh << 'BACKUP_EOF'
#!/bin/bash
BACKUP_DIR="/opt/cinestream-backups"
SERVER_DIR="/opt/cinestream"
...
BACKUP_EOF
chmod +x /opt/backup-cinestream.sh
log "Backup script created"

# Setup cron: anti-idle every 5 min + daily backup at 3 AM
mkdir -p /opt/cinestream-backups
(crontab -l 2>/dev/null | grep -v "cinestream"; echo "*/5 * * * * curl -s http://localhost:3001/health > /dev/null 2>&1"; echo "0 3 * * * /opt/backup-cinestream.sh >> /opt/cinestream-backups/backup.log 2>&1") | crontab -
