/**
 * TMDB API Service
 * Makes direct requests to The Movie Database API from the Electron main process
 * Converted from server ESM to CommonJS for Electron compatibility
 */
...
/**
 * Make a request to TMDB API using https module
...
async function getTrending(timeWindow = 'day') {
...
async function getRandomMovies() {
    ...
    const data = await tmdbFetch('/discover/movie', {
        page: randomPage, sort_by: 'popularity.desc', 'vote_average.gte': 6, 'vote_count.gte': 100,
    });
    const shuffled = data.results.sort(() => Math.random() - 0.5).slice(0, 20);
    return { results: shuffled.map(transformMovie), page: 1, totalPages: 1, totalResults: shuffled.length };
}
...
async function getMovieCredits(movieId) {
    ...
    return {
        ...
        crew: data.crew?.slice(0, 10).map(member => ({
            id: member.id, name: member.name, job: member.job,
            department: member.department, profileImage: getImageUrl(member.profile_path, 'w185'),
        })) || [],
    };
}
...
async function getMovieImages(movieId) {
    ...
    return {
        backdrops: data.backdrops?.slice(0, 20).map(img => ({
            path: getImageUrl(img.file_path, 'w780'), pathOriginal: getImageUrl(img.file_path, 'original'),
            width: img.width, height: img.height, aspectRatio: img.aspect_ratio,
        })) || [],
        posters: data.posters?.slice(0, 10).map(img => ({
            path: getImageUrl(img.file_path, 'w500'), pathOriginal: getImageUrl(img.file_path, 'original'),
            width: img.width, height: img.height, aspectRatio: img.aspect_ratio,
        })) || [],
...
async function getMovieVideos(movieId) {
    ...
    return {
        results: data.results?.map(video => ({
            id: video.id, key: video.key, name: video.name, site: video.site,
            type: video.type, official: video.official, publishedAt: video.published_at,
...
async function searchMulti(query, page = 1) {
    ...
    const results = data.results
        ...
        .map(item => {
            if (item.media_type === 'movie') return { ...transformMovie(item), mediaType: 'movie' };
            else return { ...transformTVSeries(item), mediaType: 'tv' };
        });
...
async function discoverMovies(options = {}) {
    const { page = 1, sortBy = 'popularity.desc', year, genre, minRating, maxRating } = options;
    const params = { page, sort_by: sortBy, include_adult: false, include_video: false };
...
// TV Series Methods
async function getTVDetails(seriesId) { return transformTVDetails(await tmdbFetch(`/tv/${seriesId}`)); }

async function getTVSeasonDetails(seriesId, seasonNumber) {
    const data = await tmdbFetch(`/tv/${seriesId}/season/${seasonNumber}`);
    return {
        id: data.id, name: data.name, overview: data.overview, seasonNumber: data.season_number,
        airDate: data.air_date, poster: getImageUrl(data.poster_path, 'w300'),
        episodes: data.episodes?.map(transformEpisode) || [],
    };
}

async function getSimilarTV(seriesId, page = 1) {
...
async function getTVCredits(seriesId) {
...
async function getTrendingTV(timeWindow = 'day') {
    const data = await tmdbFetch(`/trending/tv/${timeWindow}`);
    return { results: data.results.filter(m => m.poster_path).map(transformTVSeries), page: data.page, totalPages: data.total_pages, totalResults: data.total_results };
}
...
// Person Methods
async function getPersonDetails(personId) { return transformPerson(await tmdbFetch(`/person/${personId}`)); }
...
async function getPersonMovieCredits(personId) {
    ...
    return {
        cast: data.cast?.sort((a, b) => {
            ...
            return dateB - dateA;
        }).slice(0, 30).map(movie => ({ ...transformMovie(movie), character: movie.character })) || [],
        crew: data.crew?.slice(0, 10).map(movie => ({ ...transformMovie(movie), job: movie.job, department: movie.department })) || [],
    };
}
...
async function getPersonTVCredits(personId) {
    ...
    return {
        cast: data.cast?.sort((a, b) => {
            ...
            return dateB - dateA;
        }).slice(0, 30).map(show => ({ ...transformTVSeries(show), character: show.character, episodeCount: show.episode_count })) || [],
        crew: data.crew?.slice(0, 10).map(show => ({ ...transformTVSeries(show), job: show.job, department: show.department })) || [],
    };
}
...
async function getPersonImages(personId) {
    ...
    return {
        profiles: data.profiles?.map(img => ({
            path: getImageUrl(img.file_path, 'w500'), pathLarge: getImageUrl(img.file_path, 'original'),
            aspectRatio: img.aspect_ratio, width: img.width, height: img.height,
        })) || [],
    };
}
...
async function getTrendingAnime() {
    const data = await tmdbFetch('/discover/tv', { with_genres: 16, with_origin_country: 'JP', sort_by: 'popularity.desc', 'vote_count.gte': 50 });
    return { results: data.results.filter(m => m.poster_path).map(transformTVSeries), page: data.page, totalPages: data.total_pages, totalResults: data.total_results };
}

async function getPopularAnime(page = 1) {
    const data = await tmdbFetch('/discover/tv', { with_genres: 16, with_origin_country: 'JP', sort_by: 'popularity.desc', page });
    return { results: data.results.filter(m => m.poster_path).map(transformTVSeries), page: data.page, totalPages: data.total_pages, totalResults: data.total_results };
}

async function getTopRatedAnime(page = 1) {
    const data = await tmdbFetch('/discover/tv', { with_genres: 16, with_origin_country: 'JP', sort_by: 'vote_average.desc', 'vote_count.gte': 200, page });
    return { results: data.results.filter(m => m.poster_path).map(transformTVSeries), page: data.page, totalPages: data.total_pages, totalResults: data.total_results };
}

async function getAiringAnime() {
    const data = await tmdbFetch('/discover/tv', { with_genres: 16, with_origin_country: 'JP', 'air_date.gte': new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], sort_by: 'popularity.desc' });
    return { results: data.results.filter(m => m.poster_path).map(transformTVSeries), page: data.page, totalPages: data.total_pages, totalResults: data.total_results };
}

async function discoverAnime(options = {}) {
    const { page = 1, sortBy = 'popularity.desc', year, genre } = options;
    const params = { page, sort_by: sortBy, with_genres: genre ? `16,${genre}` : 16, with_origin_country: 'JP' };
...
module.exports = {
    getImageUrl,
    // Movies
    getTrending, getPopular, getTopRated, getNowPlaying, getUpcoming, getRandomMovies,
    getMovieDetails, getMovieCredits, getMovieImages, getMovieVideos,
    getSimilarMovies, getMovieRecommendations,
    searchMulti, discoverMovies, getGenres,
    // TV
    getTVDetails, getTVSeasonDetails, getSimilarTV, getTVCredits,
    getTrendingTV, getPopularTV, getTopRatedTV, getAiringTV,
    // Person
    getPersonDetails, getPersonMovieCredits, getPersonTVCredits, getPersonImages, getPersonCombinedCredits,
    // Anime
    getTrendingAnime, getPopularAnime, getTopRatedAnime, getAiringAnime, discoverAnime, getAnimeGenres,
};
